...
func start(ctx context.Context, isProd bool, nodeType, env string, ethService ethv1connect.EthServiceHandler) error {
	myEndpoint := oaenv.String("OPENAUDIO_NODE_ENDPOINT", "nodeEndpoint")
	if myEndpoint == "" {
		slog.Info("missing required env variable: OPENAUDIO_NODE_ENDPOINT. sleeping...")
		time.Sleep(time.Hour)
		log.Fatal("missing required env variable: OPENAUDIO_NODE_ENDPOINT")
	}
