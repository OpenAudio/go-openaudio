...
// A terminal status carries no schedule at all, so it is unreachable by the
...
func TestTerminalStatusesAreNeverRetried(t *testing.T) {
	...
	done := fmt.Sprintf("waveform-terminal-%d", time.Now().UnixNano())
	t.Cleanup(func() { deleteTestWaveform(t, ss, done) })
	insertTestWaveform(t, ss, done, "")
	backdateWaveformAttempt(t, ss, done, 30*24*time.Hour)

	ss.sweepWaveformRetries(ctx)
	require.NotContains(t, drainWaveformWork(ss), done,
		"a finished waveform is never re-attempted, however old the attempt")
}
