...
func migrateOps(db *gorm.DB) error {
	...
	BEGIN;
		...
		UPDATE ops SET core_tx_status = 'legacy' WHERE core_tx_status IS NULL OR core_tx_status = '';
		CREATE INDEX IF NOT EXISTS ops_core_pending_idx ON ops(host, core_tx_status, ulid);
...
func (c *Crudr) ApplyOp(op *Op) error {
	...
	if err != nil {
		return fmt.Errorf("invalid operation data: %v %s", err, op.Data)
	}
