...
var compareTables = []compareTable{
	{
		...
		Columns: []string{
			...
			// handle_lc is derived by the indexer rather than copied, and it
			// has diverged from handle before -- see the user_update.go note
			// about handle going NULL while handle_lc survived.
			"handle_lc", "is_available",
			"playlist_library", "artist_pick_track_id",
			"twitter_handle", "instagram_handle", "tiktok_handle",
			"website", "donation", "profile_type",
		...
		// profile_picture and cover_photo were immutable in the legacy indexer
		// but are updatable here. The rest are storage-v1 replica-set fields
		// and verification provenance the migration does not carry; reported
		// rather than omitted so they stay visible.
		// profile_type is an enum (profile_type_enum); pgx cannot scan an
		// unregistered enum OID into interface{}, so it has to come back as
		// text. WHERE-false validation does not catch this -- it checks that
		// the query plans, never that a value scans.
		CastCols: map[string]string{"profile_type": "profile_type::text"},
		KnownDiffs: []string{
			"profile_picture", "cover_photo",
			"metadata_multihash", "creator_node_endpoint",
			"primary_id", "secondary_ids", "replica_set_update_signer",
			"has_collectibles", "user_storage_account", "user_authority_account",
			"is_storage_v2",
	...
	},
	{
		...
		Columns: []string{
			...
			// Metadata the writer selects and emits but nothing verified
			// until now: a column dropped anywhere between the writer's
			// SELECT and the indexer's INSERT produced no count change, no
			// aggregate change, and no row mismatch.
			"isrc", "iswc", "license", "field_visibility",
			"stem_of", "remix_of", "stream_conditions", "download_conditions",
			"ai_attribution_user_id", "audio_upload_id",
			"preview_start_seconds", "release_date",
			"is_original_available", "orig_filename",
			"bpm", "musical_key", "is_custom_bpm", "is_custom_musical_key",
			"comments_disabled", "no_ai_use", "access_authorities",
			"cover_original_song_title", "cover_original_artist",
			// DDEX ingestion path -- added to the writer in #517.
			"ddex_app", "ddex_release_ids", "artists",
			"resource_contributors", "indirect_resource_contributors",
		...
		// Columns the writer does not carry across. Listed rather than
		// omitted so the run reports them every time instead of leaving
		// them invisible: an intentional drop and a silent regression look
		// identical when a column is simply absent from this file.
		// route_id/track_segments/file_type/credits_splits/metadata_multihash
		// are legacy storage-v1 fields; playlists_previously_containing_track
		// and pinned_comment_id are derived by the indexer, not migrated.
		KnownDiffs: []string{
			"duration",
			"credits_splits", "create_date", "file_type", "metadata_multihash",
			"route_id", "track_segments", "placement_hosts",
			"playlists_previously_containing_track", "allowed_api_keys",
			"audio_analysis_error_count", "pinned_comment_id", "is_owned_by_user",
		},
	...
	},
	{
		...
		Columns: []string{
			...
			"is_delete", "playlist_contents",
			"upc", "is_image_autogenerated",
			"stream_conditions", "release_date",
			"ddex_app", "ddex_release_ids", "artists",
			"copyright_line", "producer_copyright_line", "parental_warning_type",
		},
		Where:     "is_current = true AND is_delete = false",
		SampleCol: "playlist_id",
		// Legacy indexer bug: playlist_image_multihash got the sizes_multihash value during create
		// last_added_to is a legacy bookkeeping timestamp the writer does not
		// reproduce; reported rather than omitted.
		// metadata_multihash: legacy storage-v1 CID, never populated by the
		// indexer (nil on every row); already a KnownDiff on users and tracks.
		KnownDiffs: []string{"playlist_image_multihash", "last_added_to", "metadata_multihash"},
		Aggregates: []aggCheck{
			{"current", countWhere("is_current")},
			{"current_deleted", countWhere("is_current AND is_delete")},
	...
	},
	{
		...
		Columns:   []string{"is_delete", "is_save_of_repost"},
		Where:     "is_current = true",
		ProdWhere: "is_current = true",
	...
	},
	{
		...
		Columns:   []string{"is_delete", "is_repost_of_repost"},
		Where:     "is_current = true",
		ProdWhere: "is_current = true",
	...
	},
	{
		...
		Columns:   []string{"is_delete", "entity_type", "entity_id"},
		Where:     "is_current = true",
		ProdWhere: "is_current = true",
	...
	},
	{
		...
		Columns: []string{"user_id", "entity_id", "entity_type", "text", "is_delete",
			"track_timestamp_s", "is_visible", "is_edited", "is_members_only", "video_url"},
	...
	},
	{
		...
		Columns: []string{"user_id", "name", "description", "is_delete", "is_personal_access", "image_url"},
		Where:   "is_current = true",
		Aggregates: []aggCheck{
			{"current", countWhere("is_current")},
			{"current_deleted", countWhere("is_current AND is_delete")},
	...
	},
	{
		...
		CastCols:  map[string]string{"chain": "chain::text"},
		Where:     "is_current = true AND is_delete = false",
		ProdWhere: "is_current = true AND is_delete = false",
		SampleCol: "user_id",
		// Surrogate serial, assigned independently on each side; never
		// comparable. Recorded so it is not mistaken for an oversight.
		KnownDiffs: []string{"id"},
		Aggregates: []aggCheck{
			{"current", countWhere("is_current")},
			{"current_deleted", countWhere("is_current AND is_delete")},
	...
	},
	{
		...
		Columns: []string{"event_type", "user_id", "entity_type", "entity_id", "is_deleted", "end_date", "event_data"},
		Where:   "true",
		CastCols: map[string]string{
