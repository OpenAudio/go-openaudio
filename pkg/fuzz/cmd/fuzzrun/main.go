...
func runSimulatedLoop(cfg simulatedLoopConfig) error {
	...
	if err := runSimulatedEndpointRegisterRoundTrip(cfg); err != nil {
		return err
	}
	if err := runSimulatedCohortEndpointConsensusIsolation(cfg); err != nil {
		return err
	}
	if err := runSimulatedInactiveEndpointIsolation(cfg); err != nil {
		return err
	}
	if err := runSimulatedJailedEndpointRepairRoundTrip(cfg); err != nil {
		return err
	}
	...
	if err := runSimulatedJailedRegisterRoundTrip(cfg); err != nil {
		return err
	}
	...
	if err := runSimulatedMixedLifecycleQuorumRecovery(cfg); err != nil {
		return err
	}
...
func runSimulatedEndpointRepairIdempotency(cfg simulatedLoopConfig) error {
	nodeCount := clampSimNodeCount(cfg.nodes, 4)
	network, err := fuzz.NewSimulatedNetwork(fuzz.SimulatedNetworkOptions{
		NodeCount:      nodeCount,
		InitialActive:  nodeCount,
	...
	result, err := fuzz.Runner{
		...
		Seed:        cfg.seed,
		StepTimeout: cfg.stepTimeout,
	}.Run(ctx, fuzz.EndpointRepairIdempotencyScenario(network.Spec(), fuzz.ValidatorChaosController{
		EndpointMutator: network,
	}, "", cfg.window, cfg.pollInterval))
	if err != nil {
...
func runSimulatedEndpointRegisterRoundTrip(cfg simulatedLoopConfig) error {
	nodeCount := clampSimNodeCount(cfg.nodes, 4)
	network, err := fuzz.NewSimulatedNetwork(fuzz.SimulatedNetworkOptions{
		NodeCount:      nodeCount,
		InitialActive:  nodeCount,
	...
	result, err := fuzz.Runner{
		...
		Seed:        cfg.seed,
		StepTimeout: cfg.stepTimeout,
	}.Run(ctx, fuzz.EndpointRegisterRoundTripScenario(network.Spec(), fuzz.ValidatorChaosController{
		Registrar:       network,
		EndpointMutator: network,
	}, "", cfg.window, cfg.pollInterval))
	if err != nil {
		return fmt.Errorf("sim endpoint register round-trip failed seed=%d events=%d: %w", result.Seed, len(result.Events), err)
	}
	fmt.Printf("sim endpoint register round-trip ok seed=%d nodes=%d events=%d\n", result.Seed, len(network.Spec().Nodes), len(result.Events))
	return nil
}

func runSimulatedCohortEndpointConsensusIsolation(cfg simulatedLoopConfig) error {
	nodeCount := clampSimNodeCount(cfg.nodes, 4)
	network, err := fuzz.NewSimulatedNetwork(fuzz.SimulatedNetworkOptions{
		NodeCount:      nodeCount,
		InitialActive:  nodeCount,
	...
	result, err := fuzz.Runner{
		...
		Seed:        cfg.seed,
		StepTimeout: cfg.stepTimeout,
	}.Run(ctx, fuzz.CohortEndpointConsensusIsolationScenario(network.Spec(), fuzz.ValidatorChaosController{
		EndpointMutator: network,
	}, cfg.window, cfg.pollInterval))
	if err != nil {
		return fmt.Errorf("sim cohort-endpoint consensus isolation failed seed=%d events=%d: %w", result.Seed, len(result.Events), err)
	}
	fmt.Printf("sim cohort-endpoint consensus isolation ok seed=%d nodes=%d events=%d\n", result.Seed, len(network.Spec().Nodes), len(result.Events))
	return nil
}
...
func runSimulatedJailedEndpointRepairRoundTrip(cfg simulatedLoopConfig) error {
	nodeCount := clampSimNodeCount(cfg.nodes, 4)
	network, err := fuzz.NewSimulatedNetwork(fuzz.SimulatedNetworkOptions{
		NodeCount:      nodeCount,
		InitialActive:  nodeCount,
	...
	result, err := fuzz.Runner{
		...
		Seed:        cfg.seed,
		StepTimeout: cfg.stepTimeout,
	}.Run(ctx, fuzz.JailedEndpointRepairRoundTripScenario(network.Spec(), fuzz.ValidatorChaosController{
		EndpointMutator: network,
		Jailer:          network,
	}, "", cfg.window, cfg.pollInterval))
	if err != nil {
		return fmt.Errorf("sim jailed endpoint repair round-trip failed seed=%d events=%d: %w", result.Seed, len(result.Events), err)
	}
	fmt.Printf("sim jailed endpoint repair round-trip ok seed=%d nodes=%d events=%d\n", result.Seed, len(network.Spec().Nodes), len(result.Events))
	return nil
}
...
func runSimulatedJailedRegisterRoundTrip(cfg simulatedLoopConfig) error {
	nodeCount := clampSimNodeCount(cfg.nodes, 4)
	network, err := fuzz.NewSimulatedNetwork(fuzz.SimulatedNetworkOptions{
		NodeCount:      nodeCount,
		InitialActive:  nodeCount,
	...
	result, err := fuzz.Runner{
		...
		Seed:        cfg.seed,
		StepTimeout: cfg.stepTimeout,
	}.Run(ctx, fuzz.JailedRegisterRoundTripScenario(network.Spec(), fuzz.ValidatorChaosController{
		Registrar: network,
		Jailer:    network,
	}, "", cfg.window, cfg.pollInterval))
	if err != nil {
		return fmt.Errorf("sim jailed register round-trip failed seed=%d events=%d: %w", result.Seed, len(result.Events), err)
	}
	fmt.Printf("sim jailed register round-trip ok seed=%d nodes=%d events=%d\n", result.Seed, len(network.Spec().Nodes), len(result.Events))
	return nil
}
...
func runSimulatedMixedLifecycleQuorumRecovery(cfg simulatedLoopConfig) error {
	nodeCount := clampSimNodeCount(cfg.nodes, 4)
	network, err := fuzz.NewSimulatedNetwork(fuzz.SimulatedNetworkOptions{
		NodeCount:      nodeCount,
		InitialActive:  nodeCount,
	...
	result, err := fuzz.Runner{
		...
		Seed:        cfg.seed,
		StepTimeout: cfg.stepTimeout,
	}.Run(ctx, fuzz.MixedLifecycleQuorumRecoveryScenario(network.Spec(), fuzz.ValidatorChaosController{
		Registrar: network,
		Jailer:    network,
	}, cfg.window, cfg.pollInterval))
	if err != nil {
		return fmt.Errorf("sim mixed lifecycle quorum recovery failed seed=%d events=%d: %w", result.Seed, len(result.Events), err)
	}
	fmt.Printf("sim mixed lifecycle quorum recovery ok seed=%d nodes=%d events=%d\n", result.Seed, len(network.Spec().Nodes), len(result.Events))
	return nil
}
