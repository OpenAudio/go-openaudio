...
func (n *VersionSkewNetwork) RegisterNode(ctx context.Context, node NodeSpec) error {
	...
	if err := n.withNode(ctx, node.ID, func(modelNode *versionSkewNode) {
		...
		case ModelValidatorActive:
			n.triggerActiveRegisterIncompatibilityLocked()
		case ModelValidatorJailed:
			if n.mode == VersionSkewModeNoopOnJailedRegister {
				return
			}
			n.triggerJailedRegisterIncompatibilityLocked()
		}
...
func (n *VersionSkewNetwork) JailNode(ctx context.Context, node NodeSpec) error {
	return n.withNode(ctx, node.ID, func(modelNode *versionSkewNode) {
		...
		case ModelValidatorActive:
			...
			modelNode.online = false
		case ModelValidatorJailed:
			n.triggerDuplicateJailIncompatibilityLocked()
		}
	})
}
...
func (n *VersionSkewNetwork) triggerDuplicateJailIncompatibilityLocked() {
	switch n.mode {
	case VersionSkewModeForkLegacyOnDuplicateJail:
		n.diverged = true
	case VersionSkewModeHaltLegacyOnDuplicateJail:
		for id := range n.legacy {
			if node := n.nodes[id]; node != nil && node.state == ModelValidatorActive {
				node.online = false
			}
...
func (n *VersionSkewNetwork) triggerActiveRegisterIncompatibilityLocked() {
	switch n.mode {
	case VersionSkewModeForkLegacyOnActiveRegister:
		n.diverged = true
	case VersionSkewModeHaltLegacyOnActiveRegister:
		for id := range n.legacy {
			if node := n.nodes[id]; node != nil && node.state == ModelValidatorActive {
				node.online = false
			}
...
func (n *VersionSkewNetwork) triggerJailedRegisterIncompatibilityLocked() {
	switch n.mode {
	case VersionSkewModeForkLegacyOnJailedRegister:
		n.diverged = true
	case VersionSkewModeHaltLegacyOnJailedRegister:
		for id := range n.legacy {
			if node := n.nodes[id]; node != nil && node.state == ModelValidatorActive {
				node.online = false
			}
...
func (n *VersionSkewNetwork) triggerStartIncompatibilityLocked() {
	switch n.mode {
