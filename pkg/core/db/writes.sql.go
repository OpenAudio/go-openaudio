...
const jailRegisteredNode = `-- name: JailRegisteredNode :exec
update core_validators set jailed = true where comet_address = $1
`

func (q *Queries) JailRegisteredNode(ctx context.Context, cometAddress string) error {
	_, err := q.db.Exec(ctx, jailRegisteredNode, cometAddress)
	return err
}
...
const unjailRegisteredNode = `-- name: UnjailRegisteredNode :exec
update core_validators set jailed = false where comet_address = $1
`

func (q *Queries) UnjailRegisteredNode(ctx context.Context, cometAddress string) error {
	_, err := q.db.Exec(ctx, unjailRegisteredNode, cometAddress)
	return err
}
...
const upsertLegacyReplayRewardPool = `-- name: UpsertLegacyReplayRewardPool :exec
insert into core_reward_pools (
    rewards_manager_pubkey,
    authorities
) values ($1, $2)
...
type UpsertLegacyReplayRewardPoolParams struct {
	RewardsManagerPubkey string
	Authorities          []string
}
...
func (q *Queries) UpsertLegacyReplayRewardPool(ctx context.Context, arg UpsertLegacyReplayRewardPoolParams) error {
	_, err := q.db.Exec(ctx, upsertLegacyReplayRewardPool, arg.RewardsManagerPubkey, arg.Authorities)
	return err
}
