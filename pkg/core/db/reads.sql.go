...
const getActiveStorageNodeEndpoints = `-- name: GetActiveStorageNodeEndpoints :many
select endpoint
from core_validators
where node_type in ('content-node', 'validator')
`

func (q *Queries) GetActiveStorageNodeEndpoints(ctx context.Context) ([]string, error) {
	rows, err := q.db.Query(ctx, getActiveStorageNodeEndpoints)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) GetAllRegisteredNodesIncludingJailed(ctx context.Context) ([]CoreValidator, error) {
	rows, err := q.db.Query(ctx, getAllRegisteredNodesIncludingJailed)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
