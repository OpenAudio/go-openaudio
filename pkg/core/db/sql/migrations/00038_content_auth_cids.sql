...
--
-- Keyed on the user the content belongs to, never a wallet. A developer app
-- is one wallet shared by every user who granted it, so authorizing whoever
-- can act for an uploading wallet would let any of an app's users claim any
-- other's uploads. The user id is asserted at upload time, not authenticated
-- — see mediorum's upload_auth.go for why that is safe.
--
...
-- and the height already live in the attestation transaction, and duplicating
-- them would add columns no query reads.
--
