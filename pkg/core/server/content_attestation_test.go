...
// signature. Binding the user into the signed payload makes the lifted
// signature recover to a different address, which then fails the
// registered-validator check — without this, a lifted signature could credit
// an attacker's user id with the victim's cids.
func TestContentAttestationIsBoundToUser(t *testing.T) {
	ca, validator := signedAttestation(t)
	...
	if strings.EqualFold(got, validator) {
		t.Fatal("a signature bound to one user must not verify for another")
	}
}
...
func TestContentAttestationRejectionsAreDistinguishableFromStoreFailures(t *testing.T) {
	deterministic := []struct {
		...
		{"no cids", &v1.ContentAttestation{UserId: 1, ValidatorAddress: "0xv", ValidatorSignature: "0xs"}},
		{"no user id", &v1.ContentAttestation{Cids: []string{"c"}, ValidatorAddress: "0xv", ValidatorSignature: "0xs"}},
		{"empty cid", &v1.ContentAttestation{UserId: 1, Cids: []string{""}, ValidatorAddress: "0xv", ValidatorSignature: "0xs"}},
		{"no validator signature", &v1.ContentAttestation{UserId: 1, Cids: []string{"c"}, ValidatorAddress: "0xv"}},
		{"unrecoverable signature", &v1.ContentAttestation{UserId: 1, Cids: []string{"c"}, ValidatorAddress: "0xv", ValidatorSignature: "0xgarbage"}},
	}
