...
//
// Possession is the entitlement instead: a validator attests that the bytes
// were uploaded to it for a user, and a track may only name cids claimed by
// the user writing it. Forging a claim then requires having the audio, at
// which point the paywall was already moot. The user is asserted, not
// authenticated, at upload time — see mediorum's upload_auth.go for why that
// is safe and for the constraint it puts on interpreting claims.
//
