...
func attestation(validator string, cids ...string) *v1.ContentAttestation {
	return attestationFor(1, validator, cids...)
}

// attestationFor names the user the upload was made for — the claim's key.
func attestationFor(userID int64, validator string, cids ...string) *v1.ContentAttestation {
...
func TestProjectContentAttestationCidsRecordsEveryCid(t *testing.T) {
	st := newMemAuthStore()
	fu := attestation("0xVaLiDaToR", "origcid", "320cid", "previewcid")
...
func TestProjectContentAttestationCidsSkipsEmpty(t *testing.T) {
	st := newMemAuthStore()
	fu := attestation("0xvalidator", "origcid", "320cid", "")
...
func TestProjectContentAttestationCidsAllowsMultipleClaimants(t *testing.T) {
	...
	if err := projectContentAttestationCids(ctx, st, attestationFor(1, "0xv1", "cid", "320", ""), "tx1"); err != nil {
		t.Fatalf("first projection failed: %v", err)
	}
	if err := projectContentAttestationCids(ctx, st, attestationFor(2, "0xv2", "cid", "320", ""), "tx1"); err != nil {
...
func TestContentAuthRejectsClaimingAnotherUsersCid(t *testing.T) {
	...
	if err := projectContentAttestationCids(ctx, st, attestation("0xv1", "orig", "gated320", ""), "tx1"); err != nil {
		t.Fatalf("projection failed: %v", err)
	}
...
func TestContentAuthUnattestedCidGoesToFirstAsserter(t *testing.T) {
	...
	first.ClaimUnattested = true
	mustContentAuth(t, st, first)
	mustProject(t, st, first)
	...
	second.ClaimUnattested = true
	mustRejectContentAuth(t, st, second, "was not uploaded for user 2")

	edit := trackCidTx(1, 2_000_001, "0xartist", "Update", map[string]any{"track_cid": "nobody-attested-this"})
	edit.ClaimUnattested = true
...
func TestAssertedClaimNeverOverridesAnExistingOne(t *testing.T) {
	...
	decoy.ClaimUnattested = true
	mustRejectContentAuth(t, st, decoy, "was not uploaded for user 2")
	mustProject(t, st, decoy) // as finalize would apply it from an ungated proposer
...
// assertions. ClaimUnattested is how the caller says the gate is active.
...
func TestUpdateClaimsUnattestedCidOnlyForAuthorizedSigner(t *testing.T) {
	...
	stranger.ClaimUnattested = true
	if err := applyAuthProjection(ctx, st, stranger); err == nil || !isAuthValidationError(err) {
	...
	owner.ClaimUnattested = true
	mustProject(t, st, owner)
	if ok, _ := st.IsCidClaimedByUser(ctx, "new-audio", 1); !ok {
...
func TestContentAuthChecksOrigAndPreviewCids(t *testing.T) {
	...
	for _, key := range []string{"orig_file_cid", "preview_cid"} {
		...
		mustProject(t, st, userCreateTx(1, "0xartist", "artist"))
		mustProject(t, st, userCreateTx(2, "0xattacker", "attacker"))
		if err := projectContentAttestationCids(ctx, st, attestationFor(1, "0xv1", "victim-orig", "victim-320", "victim-preview"), "tx1"); err != nil {
			t.Fatalf("projection failed: %v", err)
		}
		// Give the attacker a legitimately attested track_cid so only the
		// field under test can be the reason for rejection.
		if err := projectContentAttestationCids(ctx, st, attestationFor(2, "0xv1", "own-orig", "own-320", "own-preview"), "tx1"); err != nil {
			t.Fatalf("projection failed: %v", err)
		}
...
func TestContentAuthAllowsOwnUpload(t *testing.T) {
	st := newMemAuthStore()
	mustProject(t, st, userCreateTx(1, "0xartist", "artist"))
	if err := projectContentAttestationCids(context.Background(), st, attestation("0xv1", "orig", "320", "preview"), "tx1"); err != nil {
		t.Fatalf("projection failed: %v", err)
	}
...
func TestContentAuthAllowsUploadByApprovedManager(t *testing.T) {
	...
	if err := projectContentAttestationCids(ctx, st, attestation("0xv1", "orig", "320", ""), "tx1"); err != nil {
		t.Fatalf("projection failed: %v", err)
	}
...
func TestContentAuthAllowsAudioReplacement(t *testing.T) {
	...
	mustProject(t, st, userCreateTx(1, "0xartist", "artist"))
	if err := projectContentAttestationCids(ctx, st, attestation("0xv1", "orig-v1", "320-v1", ""), "tx1"); err != nil {
		t.Fatalf("projection failed: %v", err)
	}
	if err := projectContentAttestationCids(ctx, st, attestation("0xv1", "orig-v2", "320-v2", ""), "tx1"); err != nil {
		t.Fatalf("projection failed: %v", err)
	}
...
func TestContentAuthAllowsEitherOfTwoClaimants(t *testing.T) {
	...
	for _, c := range []struct {
		...
		if err := projectContentAttestationCids(ctx, st, attestationFor(c.user, "0xv1", "orig", "shared320", ""), "tx1"); err != nil {
...
func TestContentAuthRejectsSnipeViaSharedDevApp(t *testing.T) {
	...
	if err := projectContentAttestationCids(ctx, st,
		attestationFor(1, "0xv1", "orig", "user1-320", ""), "tx1"); err != nil {
		t.Fatal(err)
	}
